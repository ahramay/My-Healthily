import React, { useState, useEffect } from 'react';
import { useHistory } from 'react-router-dom';
import { makeStyles } from '@material-ui/core/styles';
import { Palette } from 'utility/Colors/Palette';
import Box from '@material-ui/core/Box';
import Card from '@material-ui/core/Card';
import CardContent from '@material-ui/core/CardContent';
import Typography from '@material-ui/core/Typography';
import Divider from '@material-ui/core/Divider';
import Table from '@material-ui/core/Table';
import TableBody from '@material-ui/core/TableBody';
import TableCell from '@material-ui/core/TableCell';
import TableContainer from '@material-ui/core/TableContainer';
import TableHead from '@material-ui/core/TableHead';
import TableRow from '@material-ui/core/TableRow';
import Paper from '@material-ui/core/Paper';
import Chip from '@material-ui/core/Chip';
import Notification from 'styleguide/form/Notification';
import Loader from 'styleguide/Loader';
import { loggedInUser } from 'utility/sessions';
import ListDialog from 'styleguide/ListDialog';
import ButtonLinks from 'styleguide/ButtonLinks';
import { filterMapRecords } from 'utility/common';
import { usersList } from 'staticData/usersList';
import MessageModal from 'styleguide/MessageModal';

import { API_PORTFOLIOS, API_NOTIFICATION_MESSAGES } from 'services/api/config';
import { serviceQuery } from 'services/query';




const useStyles = makeStyles({
  table: {
    minWidth: 650,
    maxWidth: 1600,
    border: `solid 1px ${Palette.Grey.Light}`,
    boxShadow: 'none',
    borderRadius: 1,
    '& .MuiTableBody-root .MuiTableCell-root': {
        padding: '8px 16px',
    }
  },
});

const MemberNames = (data: any) => {
    return <Box width={1} height={1} display="flex">
        {data?.list?.map((item: any, index: number) => (
            <Chip key={index} label={item} variant="outlined" style={{ marginTop: 5, marginRight: 10, marginBottom: 5}} />
        ))}
    </Box>
}

const PortfolioListing: React.FC = () => {
    const classes = useStyles();
    const history = useHistory();
    const [notification, setNotification] = useState<any>(null);
    const [showLoader, setShowLoader] = useState<boolean>(true);
    const [fetchRecords, setFetchRecords] = useState<boolean>(true);
    
    const userData = filterMapRecords(loggedInUser?.username, usersList, 'username', 'exact');
    const companyId = userData?.[0]?.company?.id || 2;
    const [portfolios, setPortfolios] = useState<any>([]);
        
    useEffect(() => {
        if (!fetchRecords) return;
        const url = API_PORTFOLIOS;
        if (url) {
            serviceQuery(url).fetchAll().then((result) => {
                setPortfolios(result.data);
                setShowLoader(false);
            }).catch(err => {
                if(!err) return;
                setNotification([API_NOTIFICATION_MESSAGES.ALERT.ERROR, err.message]);
                setShowLoader(false);
            });
            setFetchRecords(false);
        }
    }, [fetchRecords]);

    const getMembers = (dataList: any) => {
        let list = [];
        for(let item of dataList) {
            const result = [item]?.filter((subItem: any) => subItem.company.id === companyId && subItem.role === 'client_user');
            if (result?.length > 0) {
                list.push(result?.[0]);
            }
        }
        return list;
        
    } 
    const membersDropDown = (id: string) => {
        const membersList = getMembers(usersList);
        let list = [];
        for(let item of membersList) {
            if (item.id !== parseInt(id)) {
                list.push(`${item.firstName} ${item.lastName}`);
            }
        }
        return list;
    };

    const [membersListOpen, setMembersListOpen] = useState<boolean>(false);
    const [selectedMemberValue, setSelectedMemberValue] = useState<string>('');
    const [shareData, setShareData] = useState<any>({});

    const handleClickOpen = (value: string) => {
        setSelectedMemberValue(value);
        setMembersListOpen(true);
    };

    const handleClose = (data: any) => {
        setMembersListOpen(false);
        setSelectedMemberValue('');
        setShareData({
            ...shareData,
            [data.label]: data.value,
        })
    };

    const shareStatus = (key: string) => {
        const data = shareData?.[key];
        const totalMembers = data?.members?.length || 0;
        
        if (data) {
            return data.enterprise ? <Chip label="Enterprise" color="primary" variant="default" /> : (totalMembers > 0 ? <MemberNames list={data?.members} /> : 'None');
        }
        return 'None';
    }

    const navigateToPath = (url: string) => {
        history.push(url);
    }

    const deletePortfolio = (id: string) => {
        if(!id) return;
        const url = `${API_PORTFOLIOS}`;
        if (url) {
            serviceQuery(url).delete(id).then(async (result) => {
                const portfolio = await result.data;
                if (portfolio) {
                    setNotification([API_NOTIFICATION_MESSAGES.ALERT.SUCCESS, API_NOTIFICATION_MESSAGES.RECORD_DELETED_SUCCESS]);
                }
            }).catch(err => {
                if(!err) return;
                setNotification([API_NOTIFICATION_MESSAGES.ALERT.ERROR, err.message]);
            });
			setTimeout(() => {
				setFetchRecords(true);
			}, 300);
        }
    }

    const [openMessageModal, setOpenMessageModal] = useState<boolean>(false);
    const [selectedModalValue, setSelectedModalValue] = useState<string>('');
    const [selectedModalData, setSelectedModalData] = useState<string>('');

    const handleMessageModalOpen = (data: any) => {
        setSelectedModalValue(`Are you sure, you want to delete ${data?.name}?`);
        setSelectedModalData(data?.id);
        setOpenMessageModal(true);
    };

    const handleMessageModalClose = (value: string) => {
        if (value) {
            deletePortfolio(selectedModalData);
        } else {
            setSelectedModalData('');
        }
        setOpenMessageModal(false);
    };

    return (
        <>
            <Card>
                <Box p={2.5}>
                    <CardContent>
                        {notification && notification?.length && 
                            <Box my={1}>
                                <Notification severity={notification?.[0]} message={notification?.[1]} />
                            </Box>
                        }
                        <Box display="flex">
                            <Box p={1} width="100%">
                                <Typography variant="h5" component="h2">Portfolios</Typography>
                            </Box>
                            <Box p={1} flexShrink={1}>
                                { /* <Button size="small" variant="outlined" color="primary" onClick={() => navigateToPath('/portfolios/add')} >
                                    <ButtonIcon icon="add" label="ADD" />
								</Button> */ }
								<ButtonLinks label="Add" icon="add" bgColor={Palette.deepGreen.Dark} onClickAction={() => navigateToPath('/portfolios/add')}  />
                            </Box>
                        </Box>
                        <Divider />

                        {showLoader && <Loader margin={2} /> }

                        {!showLoader && 
                            <Box alignItems="center" py={2}>
                                <TableContainer className={classes.table} component={Paper}>
                                    <Table aria-label="scenarios table">
                                        <TableHead>
                                            <TableRow>
                                                <TableCell align="left" width="30%"><b>Name</b></TableCell>
                                                <TableCell align="left" width="30%"><b>Shared With</b></TableCell>
												<TableCell align="left" width="10%"><b>Asset Count</b></TableCell>
                                                <TableCell align="left" width="30%"></TableCell>
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            {!showLoader && portfolios && portfolios?.length === 0  &&  
                                                <TableRow>
                                                    <TableCell component="th" scope="row" align="left">Sorry, no portfolios yet.</TableCell>
                                                </TableRow>
                                            }
                                            {portfolios?.map((row: any, key: number) => (
                                                <TableRow key={key}>
                                                    <TableCell component="th" scope="row" align="left">{row?.name}</TableCell>
                                                    <TableCell component="th" scope="row" align="left">{shareStatus(row?.id)}</TableCell>
													<TableCell component="th" scope="row" align="left">{row?.assetCount || '0'}</TableCell>
                                                    <TableCell component="th" scope="row" align="right">
                                                        <ButtonLinks label="SHARE" icon="share" bgColor={Palette.deepPurple.Dark} onClickAction={() => handleClickOpen(row?.id)}  />
                                                        <ButtonLinks label="EDIT" icon="edit" bgColor={Palette.deepPurple.Dark} onClickAction={() => navigateToPath(`/portfolios/edit/${row.id}`)}  />
                                                        <ButtonLinks label="DEL" icon="delete" circleIcon={true} color={Palette.Red.Main} bgColor={Palette.deepMaroon.BgLight} onClickAction={() => handleMessageModalOpen(row)} />
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                                </TableContainer>
                            </Box>
                        }
                    </CardContent>
                </Box>
                <ListDialog 
                    data={membersDropDown(userData?.[0]?.id || '')} selectedValue={selectedMemberValue}
                    id="members-dialog" title="Select Enterprise" noListLabel="Sorry, no members found"
                    open={membersListOpen} onClose={handleClose} icon="person" multiCheck={true}
                />
                <MessageModal title="Confirm" message={selectedModalValue} open={openMessageModal} onClose={handleMessageModalClose} />
            </Card>
        </>
    )
};

export default PortfolioListing;